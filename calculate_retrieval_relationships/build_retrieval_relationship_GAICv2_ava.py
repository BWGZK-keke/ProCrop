...
def search_samples():
    gaic_data_dir = "/home/kzhang99/GAICv2_train_dataset/embedding_db/"
    ava_dir = "/home/kzhang99/AVA_embedding_db/"
    ...
    # vectors_GAIC_test = np.load("/home/zhangke/RALF_code/features_GAIC_test_compressed.npy")
    # vectors_GAIC = np.load("/home/zhangke/RALF_code/features_GAIC_train_compressed.npy")

    # with open('kmeans_compressed.pickle', 'rb') as f:
    #     model = pickle.load(f)
    # with open('umap_compressed.pickle', 'rb') as f:
    #     trans = pickle.load(f)

    # labels_saved = np.load("/home/zhangke/RALF_code/labels_umap_16_3d_gaic.npy")
    ava_dataset["train"].load_faiss_index('embeddings', '/home/kzhang99/RALF_code/cache/ava_cache_faiss/cgl_dreamsim_wo_head_index.faiss')
    ...
    for key in ["train"]:
        ...
        for i in range(len(gaic_dataset[key])):  
            ...
            scores, retrieved_examples = ava_dataset["train"].get_nearest_examples('embeddings',embedding_target, k=10)
            retrieved_name = retrieved_examples["id"]
            if i > 2:
                end_time = time.time()
                count +=1
    ...
    output_dir = "/home/kzhang99/ava_GAICv2_train_hdfs_embed_root/"
