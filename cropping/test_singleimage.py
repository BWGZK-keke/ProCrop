...
from dataset.custom_dataset import build_dataset
...
def get_args_parser():
    ...
    parser.add_argument('--dataset_root', default='',
                        help='Root directory of the GAIC dataset (images/ and annotations/ subdirs)')
    parser.add_argument('--retrieval_cache_dir', default='',
                        help='Directory with precomputed GAIC retrieval tables (.pt files)')
    parser.add_argument('--retrieval_img_dir', default='',
                        help='Directory of CGL retrieval database images')
    ...
    parser.add_argument('--crop_savepath', default='./crop_results',
...
def retrieve(sample_path, retrieval_img_dir, retrieval_cache_dir, split="test"):
    ...
    matched_names = [os.path.join(retrieval_img_dir, id2name[i]) for i in table_indexes[img_name]]
...
def main(args):
    ...
    for image_id, resdict in results.items():
        ...
        img = cv2.imread(os.path.join(args.dataset_root, str(image_id) + '.png'))

        annotation_savepath = os.path.join(args.crop_savepath,
                                           str(image_id) + '_' + 'annotation' + '.png')
        #retrieve:
        retrieved_paths = retrieve(
            os.path.join(args.dataset_root, str(image_id) + '.png'),
            args.retrieval_img_dir, args.retrieval_cache_dir
        )[0:16]
        for retrieved_path in retrieved_paths:
            image_retrieve = cv2.imread(retrieved_path)
            retrieve_anno_path = os.path.join(
                args.dataset_root, 'annotations', retrieved_path.split("/")[-1].replace(".png", ".txt")
            )
            with open(retrieve_anno_path, 'r') as fid:
                annotations_txt_retrieve = fid.readlines()      
            # print(target_retrieve.keys())
            select_index = np.random.choice([i for i in range(len(target_retrieve['good_boxes']))],1)[0]
            mask = torch.zeros_like(image_retrieve)
            image_retrieve = image_retrieve * mask
