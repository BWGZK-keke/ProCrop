...
class flms_evaluator(object):
    ...
    def update(self, batchresults, batchgt):
        ...
        self.cls_score_all = self.cls_score_all.type_as(self.cls_score_all).cuda()
    ...
    def post_process_gt(self, batchgt):
        ...
        for datadict in batchgt:
            ...
            datadict['good_boxes'] = torch.clamp(datadict['good_boxes'], min=0)
            datadict['good_boxes'][:,2] = torch.clamp(datadict['good_boxes'][:,2], max=w)
            datadict['good_boxes'][:,3] = torch.clamp(datadict['good_boxes'][:,3], max=h)
            
            processed_batchgt.append(datadict)
            self.gt_good_num["gt"] += datadict['good_boxes'].shape[0]
    ...
    def boxes_cls_thresholds(self):
        ...
        # print(self.gt_good_num['gt'])
        # print("gt good:",   torch.tensor(self.gt_good_num['gt'], dtype=torch.int16))

        # print("scores_all:", scores_all.device)
        self.gt_good_num = self.gather_final_results({'gt':  torch.tensor(self.gt_good_num["gt"], dtype=torch.int16).cuda()})
    ...
    def disp_pr_gt(self):
        ...
        for idx, (imgid, boxes_data) in enumerate(self.results.items()):
            ...
            pr_boxes = boxes_data['boxes'].type_as(gt_good_boxes)
            disp = []
            for i in range(gt_good_boxes.shape[0]):
                # iou_mat = ops.box_iou(pr_boxes, gt_good_boxes)
                disp_mat_current = ((abs(gt_good_boxes[i:i+1,0] - pr_boxes[:,0])+ abs(gt_good_boxes[i:i+1,2] - pr_boxes[:,2])) /self.gt[idx]["orig_size"][1] + (abs(gt_good_boxes[i:i+1,1]-pr_boxes[:,1])+  abs(gt_good_boxes[i:i+1,3] - pr_boxes[:,3])) /  self.gt[idx]["orig_size"][0])
                # print(self.gt[idx]["orig_size"], gt_good_boxes[i:i+1], pr_boxes[0:1])
                disp.append(disp_mat_current.unsqueeze(1))
            disp = torch.cat(disp, dim=-1)
            # print(disp.shape)
            

            pr_gt_ioudict[imgid] = disp/4
    ...
    def iou_for_threshes(self):
        ...
        pr_gt_dispdict = self.disp_pr_gt() 
        ...
        for idx, (imgid, thresh_dict) in enumerate(self.cls_results_id.items()):
            ...
            pr_gt_dispmat_eachimg = pr_gt_dispdict[imgid]
            ...
            mindisp_for_each_pr = pr_gt_dispmat_eachimg [:,maxiou_indices_gt]
            ...
            for thresh, pr_good_boxes_id in thresh_dict.items():
                ...
                if pr_good_num > 0:
                    ...
                    iou_eachimg[thresh] = torch.as_tensor(0).type_as(maxiou_clsthresh_current).cuda()
    ...
    def iou_for_nums(self):
        ...
        for idx, (imgid, value) in enumerate(self.results.items()):
            ...
            for topk_id in range(1, boxes_all.shape[0] + 1):
                ...
                # print(pr_gt_ioumat_eachimg[0])
                current_ioumat = pr_gt_ioumat_eachimg[:topk_id]
                current_dispmat = pr_gt_dispmat_eachimg[:topk_id]
                maxiou_for_each_pr, maxiou_indices_gt = torch.max(current_ioumat, dim=1)
                maxdisp_for_each_pr, _= torch.min(current_dispmat, dim=1)#current_dispmat[:,maxiou_indices_gt]
                iou_eachimg_max[topk_id] = torch.max(maxiou_for_each_pr)
                iou_eachimg_mean[topk_id] = torch.mean(maxiou_for_each_pr)
                disp_eachimg_max[topk_id] = torch.min(maxdisp_for_each_pr)#maxdisp_for_each_pr[torch.argmax(maxiou_for_each_pr)]
                disp_eachimg_mean[topk_id] = torch.mean(maxdisp_for_each_pr)

            self.iou_mean_nums[imgid] = iou_eachimg_mean
            self.iou_max_nums[imgid] = iou_eachimg_max
            self.disp_mean_nums[imgid] = disp_eachimg_mean
            self.disp_max_nums[imgid] = disp_eachimg_max
    ...
    def summary(self):
        ...
        self.iou_mean_final = torch.tensor(0.0).cuda()
        self.evalnum = 0
        ...
        for imgid, ioudict_mean in self.iou_mean_nums.items():
            ...
            for idx in range(1, boxnum + 1):
                ...
                final_disp_max_dict[idx] += self.disp_max_nums[imgid][idx]
        ...
        for num, iou_mean in final_iou_mean_dict.items():
            ...
            final_disp_max_dict[num] = final_disp_max_dict[num] / self.evalnum['evalnum']

        return final_iou_max_dict, final_iou_mean_dict, final_disp_max_dict
    ...
    def save_cropping_results(self, dataset_path, epoch, only_bbox=True):
        ...
        for image_id, boxdict in self.cls_results.items():
            ...
            # print("id:", image_id, "len:",len(self.pathlist), self.pathlist[0])
            imgname = self.pathlist[image_id].split('/')[-1].split('.')[0]+".jpg"
            index = image_id
            # image_names = [i.split('/')[-1].split('.')[0] for i in self.pathlist]
            # index = image_names.index(str(image_id))
            # imgname = str(image_id)+".png"
            

            pr_good_bbox = boxdict[self.default_cls_thresh]

            pr_cls_score_sorted, sort_id = self.cls_results_score[image_id][self.default_cls_thresh].sort(descending=False)
            ...
            for idx, score in zip(sort_id, pr_cls_score_sorted):
                ...
                if idx < 4:
                    if idx == 0:
                        ...
                        cv2.putText(img, str(round(score.item(),3)), (xmin, ymin - 5), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 0, 255), 1)
                    if idx == 1:
                        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (254, 228, 135), 2)
                        cv2.putText(img, str(round(score.item(),3)), (xmin, ymax - 5), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (254, 228, 135), 1)
                    if idx == 2:
                        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (223, 104, 0), 2)
                        cv2.putText(img, str(round(score.item(),3)), (xmin, ymin - 5), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (223, 104, 0), 1)
                    if idx == 3:
                        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (80, 150, 215), 2)
                        cv2.putText(img, str(round(score.item(),3)), (xmin, ymax - 5), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (80, 150, 215), 1)
            try:
