...
class ConditionalDETR(nn.Module):
    ...
    def forward(self, samples, samples_retrieve):
        ...
        for i in range(samples_retrieve.shape[1]):
            if isinstance(samples_retrieve, (list, torch.Tensor)):
                print(samples.shape)
                print(samples_retrieve[:,i].shape)
                sample_retrieve = nested_tensor_from_tensor_list(samples_retrieve[:,i])
                features_retrieve, pos_retrieve = self.backbone(sample_retrieve)
                print(features_retrieve.shape)

        src, mask = features[-1].decompose()
        src_retrieve, mask_retrieve = features_retrieve[-1].decompose()
        assert mask is not None
        src = torch.cat([src,src_retrieve],2)
        mask = torch.cat([mask,mask_retrieve],2)
        pos = torch.cat([pos[-1],pos_retrieve[-1]],2)
...
class ConditionalDETR_distill(nn.Module):
    ...
    def forward(self, samples, samples_retrieve):
        return self.conditionalDETR(samples, samples_retrieve)
